...
def show_fits(path_coords, path_coords_sky, mom_map, bmin, bmaj, bpa,
    ...
    fits_fig = aplpy.FITSFigure(rgb2d, figure=fig_size,
                 ...
                 fig_prop['general']['w'],
                 fig_prop['general']['h']])
    fits_fig.show_contour(mom_map, aspect='auto', levels=contour_level, colors='#ffdb58') 
...
def process_fits(base_filename, x1, y1, x2, y2, x3, y3):
    try:
        ...
        subprocess.run(["imsub", f"in={base_filename}", f"out={base_filename}_imsub2", f"region='{region_spec}'"], check=True)

        # Convert processed image back to FITS format
        subprocess.run(["fits", f"in={base_filename}_imsub2", "op=xyout", f"out={base_filename}_imsub2.fits"], check=True)

        # Remove intermediate data files
        subprocess.run(["rm", "-r", f"{base_filename}_imsub2"], check=True)
...
def update_parfile(parfile, output_path, datacube, weight_cube):
    ...
    updated_parfile = os.path.join(output_path, datacube_name.split('_')[0] + '_sofia_updated_vopt.par')
    ...
    with open(parfile, 'r') as filein, open(updated_parfile, 'w') as fileout:
        ...
        lines = lines.replace('weight_cube', weight_cube[:-5]+"_imsub2.fits")
        fileout.write(lines)
...
if __name__ == '__main__':
    ...
    cube_copy = os.path.basename(cube_or)[:10] + "_central.fits"
    weight_copy = os.path.basename(weight_or)[:-5] + "_central.fits"
    ...
    cut_coord = [("22:02:39.36", "-32:06:24"), ("22:01:39.65", "-31:49:25")]
    ...
    subprocess.run(["mv", cube_copy[:-5]+"_imsub2.fits", result_path], capture_output=True, text=True)
    subprocess.run(["mv", weight_cube[:-5]+"_imsub2.fits", result_path], capture_output=True, text=True)
    datacube_name = cube_copy[:-5]+"_imsub2"
    ...
    hdr_cube = fits.open(result_path + cube_copy)[0].header
    bmin = hdr_cube["BMIN"]
    bmaj = hdr_cube["BMAJ"]
    mom0_central = sofia_path + cube_copy[:-5]+"_imsub2_mom0.fits"
    hdu_mom0_central = fits.open(mom0_central)[0]
    img_mom0_central = hdu_mom0_central.data
    snmap = signal_to_noise_map(sofia_path + cube_copy[:-5]+"_imsub2_noise.fits",
                                mom0_central, sofia_path + cube_copy[:-5]+"_imsub2_chan.fits")
    snmap_central = fits.open(snmap)[0].data
    mask = (snmap_central >= 2.5) & (snmap_central <= 3.5)
    nhi = mom0tonhi(img_mom0_central, bmin*3600, bmaj*3600)
    nhi_fits = sofia_path + cube_copy[:-5]+"_imsub2_nhi.fits"
    fits.writeto(nhi_fits, nhi, hdu_mom0_central.header, overwrite=True)
    ...
    hdus = [hdu_mom0_central]
    cubes = [result_path + cube_copy[:-5]+"_imsub2.fits"]
    beamsize = [hpbw]
    pv_outs = [result_path + cube_copy[:-5]+"_imsub2_pvdiag.fits"]
    output = "results/publication/figures/hcg90_segmented_pv_view_mom0.pdf"
    figure_names = [output[:-14]+".pdf"]
    beampos = [(4, 174)]
    textlabel = [""]
...
#python workflow/scripts/plot_segmented_pvdiag_hcg90.py -c results/data/hcg90/hcg90_line60_masked.pb_corr_vopt.fits -m results/sofia_pbc/hcg90/hcg90_line60_masked.pb_corr_vopt_mom0.fits
